...
class DoadoraInicioScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final coletas = MockData.coletas.where((c) => c.doadoraId == doadora.id).toList();
    final concluidas = coletas.where((c) => c.status == StatusColeta.concluida).toList();
    final totalL = concluidas.fold<int>(0, (s, c) => s + (c.volumeMl ?? 0)) / 1000.0;
    final agendadas = coletas.where((c) => c.status == StatusColeta.agendada).toList();
        ...
        children: [
          Row(
            ...
            children: [
              ...
              ClBadge(doadora.statusAvaliacao.label, tone: doadora.statusAvaliacao.toBadgeTone()),
            ],
          ),
                  ...
                  child: Column(
                    ...
                    children: [
                      Text('${totalL.toStringAsFixed(1)} L',
                          style: const TextStyle(
                              fontWeight: FontWeight.w800, fontSize: 22, color: AppColors.brand)),
                  ...
                  child: Column(
                    ...
                    children: [
                      Text('${concluidas.length}',
                          style: const TextStyle(
                              fontWeight: FontWeight.w800, fontSize: 22, color: AppColors.brandDark)),
          ...
          const Text('Próxima coleta',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 17, color: AppColors.brandDark)),
          const SizedBox(height: 8),
          if (proxima != null)
            ClCard(
            ...
            ClCard(
              child: Column(
                ...
                children: [
                  ...
                  ClPrimaryButton(texto: 'Sinalizar coleta', onPressed: aoAbrirNovaColeta),
                ],
              ),
            ),
