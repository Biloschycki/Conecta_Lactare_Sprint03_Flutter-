...
class _DoadoraColetasScreenState extends State<DoadoraColetasScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final coletas = MockData.coletas
        .where((c) => c.doadoraId == doadora.id)
        .toList()
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          const Text('Minhas coletas',
              style: TextStyle(
                  fontWeight: FontWeight.w800,
                  fontSize: 22,
                  color: AppColors.brandDark)),
          const Text('Acompanhe suas doações.',
              style: TextStyle(fontSize: 12, color: AppColors.brand)),
          const SizedBox(height: 12),
          ClPrimaryButton(
              texto: '+ Sinalizar coleta', onPressed: widget.aoNovaColeta),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(
          ...
          Expanded(
            child: coletas.isEmpty
                ? const Text('Você ainda não tem coletas.',
                      ...
                      return ClCard(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text('Modalidade: ${coleta.modalidade.label}',
                                style: const TextStyle(
                                    fontSize: 12, color: AppColors.brandDark)),
                            if (coleta.volumeMl != null)
                              Text('Volume: ${coleta.volumeMl} ml',
                                  style: const TextStyle(
                                      fontSize: 12,
                                      color: AppColors.brandDark)),
